...
<body>
  ...
  <script>
    ...
    const = list = [];
    const inventors50 = inventors.filter(el => {
      if(el["year"] >= 1500 && el['year'] < 1600) list.push(el)
    });
    ...
    return inventors.map(item => ({first: item['first'], last: item['last']}));
